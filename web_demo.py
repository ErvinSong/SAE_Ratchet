...
from models.transformer import Transformer, default_hparams
...
def evaluate(inp_img, tokenizer, transformer, temperature, top_k, top_p, options, seed, MAX_LENGTH=128):
    ...
    for i in tqdm.tqdm(range(MAX_LENGTH)):
        count += 1
        name = f"log_probs/log_probs{count}.txt"
        file = open(name, "w")
        ...
        word_log_probabilities = {tokenizer.decode([i]): log_prob.numpy() for i, log_prob in enumerate(tf.squeeze(log_probabilities))}
        blegh = {k: v for k, v in sorted(word_log_probabilities.items(), key=lambda item: item[1])}
        file.write(str(blegh))
        file.close()
...
def main():
    ...
    if uploaded_file:
        ...
        img = skimage.color.gray2rgb(img) #converts our single channel to the three channels
